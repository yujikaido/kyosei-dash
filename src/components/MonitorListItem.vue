...
export default {
    ...
    methods: {
        ...
        /**
         * Toggle selection of monitor. Kyosei Dash: ticking a group also
         * (de)selects every descendant — works recursively for nested groups.
         * @returns {void}
         */
        toggleSelection() {
            const turningOn = !this.isSelected(this.monitor.id);
            const apply = (id) => {
                if (turningOn) {
                    this.select(id);
                } else {
            ...
            apply(this.monitor.id);

            // If this is a group, walk its descendants in $root.monitorList
            if (this.monitor.type === "group") {
                const stack = [ this.monitor.id ];
                const seen = new Set([ this.monitor.id ]);
                while (stack.length) {
                    const parentId = stack.pop();
                    for (const m of Object.values(this.$root.monitorList || {})) {
                        if (m.parent === parentId && !seen.has(m.id)) {
                            seen.add(m.id);
                            apply(m.id);
                            if (m.type === "group") stack.push(m.id);
                        }
                    }
                }
            }
