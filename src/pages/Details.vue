    ...
    <transition name="slide-fade" appear>
        <div v-if="monitor">
            ...
            <div v-if="monitor.connections && monitor.connections.length" class="connections-row mt-2 mb-2">
                ...
                <!-- Kyosei Dash — first-time-setup helper for the rdp:// URL handler on Windows -->
                <a
                    v-if="hasRdpConnection"
                    href="/handlers/rdp-handler.reg"
                    download="kyosei-rdp-handler.reg"
                    class="btn btn-sm btn-link text-muted mb-1"
                    title="One-time setup: download and run this .reg file on Windows so RDP buttons launch mstsc.exe"
                >
                    <font-awesome-icon icon="download" /> Windows RDP setup (first time)
                </a>
            </div>
            <p class="url">
                <a
            ...
            <div class="functions">
                <div class="btn-group" role="group">
                    <!-- Kyosei Dash — pin/unpin to dashboard -->
                    <button
                        class="btn btn-normal"
                        :class="{ 'btn-primary text-white': monitor.pinned_to_dashboard }"
                        :title="monitor.pinned_to_dashboard ? 'Pinned to dashboard — click to unpin' : 'Pin to dashboard'"
                        @click="togglePin"
                    >
                        <font-awesome-icon icon="thumbtack" />
                        {{ monitor.pinned_to_dashboard ? "Pinned" : "Pin to dashboard" }}
                    </button>
...
export default {
    ...
    computed: {
        ...
        /** Kyosei Dash — true if any connection is rdp type (drives the .reg helper link) */
        hasRdpConnection() {
            const conns = (this.monitor && this.monitor.connections) || [];
            return conns.some((c) => c.type === "rdp");
        },
    ...
    methods: {
        ...
        /**
         * Kyosei Dash — toggle this monitor's dashboard pin.
         * @returns {void}
         */
        togglePin() {
            const next = !this.monitor.pinned_to_dashboard;
            this.$root.getSocket().emit("kyoseiSetPinned", this.monitor.id, next, (res) => {
                if (res && res.ok) {
                    this.$root.toastSuccess(next ? "Pinned to dashboard." : "Unpinned.");
                } else {
